...
public class JobPropertyFactory {
	...
	public static JobProperty newJobProperty(
		boolean useBasePropertyName, String basePropertyName,
		String testSuiteName, String ruleName, Job job, File testBaseDir,
		...
		if (type == JobProperty.Type.DEFAULT_TEST_DIR) {
			jobProperty = new DefaultTestDirProperty(
				job, type, testBaseDir, basePropertyName, testSuiteName,
				ruleName, useBasePropertyName);
		}
			...
			jobProperty = new DefaultJobProperty(
				job, type, basePropertyName, testSuiteName, ruleName,
				useBasePropertyName);
		}
